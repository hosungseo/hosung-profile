...
export function exportDraft(context,draft,datasets,brief){
 ...
 const lines=['# 특공대 · 특례·데이터 활용 묶음','',`- 지역: ${literal(context.region)}`,`- 탐색 주제: ${literal(context.topic.short)}`,`- 개별 특례: ${literal(c.zoneName+' · '+c.title)}`,`- 특례 식별자: ${literal(c.id)}`,'- 문서 성격: 사용자 작성 초안 · 기관 미제출 · 기업 수요 진위 미확인',`- 내보낸 시각: ${new Date().toISOString()}`,'','## 1. 내가 수행할 실증·사업화 과제','',input(draft.goal),'','## 2. 선택한 개별 특례 · 보유 자료 기준','',`**기존 제한:** ${literal(c.before)}`,'',`**특례로 달라지는 것:** ${literal(c.after)}`,'',`**남는 조건:** ${literal(c.conditions)}`,'',`- 대상: ${literal(c.audience)}`,`- 기간: ${literal(c.period)}`,`- 출처 상태: ${literal(c.status)}`,`- 근거: ${safeLink(c.source,c.sourceTitle)}`,'','현재 효력·신규 참여 자격은 별도 확인 대상입니다. 아래 자료는 주제 수준의 큐레이션 후보이며, 개별 특례에 적합한지는 직접 대조해야 합니다.','',`## 3. 직접 담은 데이터 ${draft.selected.length}개`,''];
